...
if __name__ == "__main__":
    ...
    if load_config_flag and load_codebook_flag and load_model_flag:
        ...
        while True:
            ...
            _pth = r"D:\CMC\DeepVision\recorded_data\validation\ar_30_5_random_00\video.mp4"
            if is_video:
                if cap is None: cap = cv2.VideoCapture(_pth)
                ret, image =cap.read()
                
                # image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
                # flip image to 90 degree
            ...
            # print(stag_image_processor.bbox_corner_info)
            
            decoded_tags, roi, bbox = stag_image_processor.process2(image, detect_scale=None)
